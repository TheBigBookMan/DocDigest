echo "Starting build and deployment of DocDigest lambda processor"

# Load .env file if it exists
if [ -f ../lambda/processor/.env ]; then
  export $(grep -v '^#' ../lambda/processor/.env | xargs)
fi
...
echo "Starting docker image build for lambda processor"
docker build -t ${ECR_REPO_NAME}:${GIT_SHA} ../lambda/processor
echo "Finished building lambda processor image"
